            ...
            try:
                ...
                try:
                    ...
                    if result:
                        ...
                        messages.success(
                            request,
                            f'Receipt processed with OCR! Found: {receipt.merchant_name or "Unknown merchant"}',
                        )
                    else:
                    ...
                    messages.warning(
                        request,
                        f"OCR error: {str(ocr_error)}. Receipt saved without processing.",
                    )
...
def create_transaction_from_receipt(request, receipt_id):
    ...
    if receipt.status != "completed":
        messages.error(
            request,
            "This receipt hasn't been processed yet. Please wait for OCR processing to complete.",
        )
    ...
    if not receipt.total_amount:
        messages.error(
            request,
            "No amount detected in this receipt. Please add the transaction manually.",
        )
    ...
    try:
        ...
        if existing_transaction:
            messages.warning(
                request, "A transaction has already been created from this receipt."
            )
        ...
        if not account:
            messages.error(
                request, "No active account found. Please create an account first."
            )
        ...
        success_msg = (
            f"✅ Transaction created successfully! Amount: £{receipt.total_amount}"
        )
